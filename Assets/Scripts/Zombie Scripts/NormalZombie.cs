...
public class NormalZombie : MonoBehaviour
{
    ...
    private void FollowPlayer()
    {
        ...
        if (distanceToPlayer <= enemyDetectionRange)
        {
            // Set the destination to the player's position
            navAgent.SetDestination(player.position);
            animator.SetInteger("NormalZombieAnimations", 1); // Set to walk animation
        }
        else
        {
            animator.SetInteger("NormalZombieAnimations", 0); // Set to idle animation
        }
    }
    ...
    private void AttackPlayer()
    {
        ...
        if (distanceToPlayer <= enemyAttackRange && Time.time >= nextAttackTime)
        {
            ...
            animator.SetInteger("NormalZombieAnimations", 2); // Set to attack animation
