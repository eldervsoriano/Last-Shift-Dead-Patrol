...
public class VirtualJoystick : MonoBehaviour, IDragHandler, IPointerDownHandler, IPointerUpHandler
{
    ...
    public void OnDrag(PointerEventData eventData)
    {
        ...
        if (RectTransformUtility.ScreenPointToLocalPointInRectangle(joystickBackground, eventData.position, eventData.pressEventCamera, out position))
        {
            ...
            // Apply movement speed to the joystick handle position
            joystickHandle.anchoredPosition = new Vector2(
                InputDirection.x * (joystickBackground.sizeDelta.x / 2),
                InputDirection.z * (joystickBackground.sizeDelta.y / 2)) * moveSpeed;
        }
    }
    ...
    // Properties to get the movement direction with applied speed
    public float Horizontal => InputDirection.x * moveSpeed;
    public float Vertical => InputDirection.z * moveSpeed;

    // Method to upgrade movement speed
    public void UpgradeMovementSpeed(float additionalSpeed)
    {
